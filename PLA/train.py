...
dataloader_train = torch.utils.data.DataLoader(AG_dataset_train, shuffle=True, num_workers=conf.num_workers,batch_size = 3,
...
AG_dataset_test = AG(mode="test", datasize=conf.datasize, data_path=conf.data_path, ws_object_bbox_path=None, remove_one_frame_video=True,
                     filter_nonperson_box_frame=True, filter_small_box=False if conf.mode == 'predcls' else True)
dataloader_test = torch.utils.data.DataLoader(AG_dataset_test, shuffle=False, num_workers=conf.num_workers, batch_size = 3,
                                              collate_fn=cuda_collate_fn, pin_memory=False)
        ...
        for b in range(len(dataloader_train)):
            ...
            if entry != None:
                ...
                if not conf.bce_loss:
                    ...
                    if conf.loss == 'KL':
                        # print(spatial_distribution, spatial_label) # TODO
                        losses["spatial_relation_loss"] = kl_loss(F.log_softmax(spatial_distribution, dim=1),
                                                                    F.softmax(spatial_label, dim=1))
                        losses["contact_relation_loss"] = kl_loss(F.log_softmax(contact_distribution, dim=1),
                                                                    F.softmax(contact_label, dim=1))
                    elif conf.loss == 'L1':
